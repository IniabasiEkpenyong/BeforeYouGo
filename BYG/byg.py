...
def add_subtask():
    ...
    with sqlalchemy.orm.Session(database._engine) as session_db:
        ...
        if ub_item:
            new_subtask = database.SubTask(
                user_bucket_id=user_bucket_id,
                description=description,
                completed=False
            )
            session_db.add(new_subtask)
            session_db.commit()
...
def toggle_subtask():
    ...
    subtask_id = flask.request.form.get('subtask_id')
    if not subtask_id:
        return flask.redirect('/my_bucket')
    ...
    with sqlalchemy.orm.Session(database._engine) as session_db:
        ...
        if subtask:
            ...
            user_bucket = session_db.query(UserBucket).filter_by(id=subtask.user_bucket_id).first()
            if user_bucket:
                all_subtasks = session_db.query(database.SubTask).filter_by(
                    user_bucket_id=user_bucket.id).all()
                
                if all_subtasks and all(st.completed for st in all_subtasks):
